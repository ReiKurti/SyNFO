import winreg, os, time, sys, getpass, winsound
from colorama import init, Fore, Back, Style
init()
os.system('CLS')
newpath = r'C:/Users/'+getpass.getuser()+'/Desktop/SyNFO OUTPUT'
if not os.path.exists(newpath):
    os.makedirs(newpath)
# PRETITOLO ------------------------------------------------------------------------------------------
print()
print(Fore.WHITE+Style.BRIGHT+' SyNFO is a simple System Information Recovery Tool that allows you to know'+Fore.RESET)
print(Fore.WHITE+Style.BRIGHT+' the information of your PC, BIOS, Video Card and Processor with a click.'+Fore.RESET)

# TITOLO ---------------------------------------------------------------------------------------------
print()
print(Fore.MAGENTA+Style.BRIGHT+' SyNFO 0.8'+Fore.RESET)
print()

# PC INFO ---------------------------------------------------------------------------------------------
print(Fore.CYAN+Style.BRIGHT+'--------------------'+Fore.RESET)
print(Fore.YELLOW+Style.BRIGHT+' PC INFO'+Fore.RESET)
print(Fore.CYAN+Style.BRIGHT+'--------------------'+Fore.RESET)

hKey = winreg.OpenKey (winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows NT\CurrentVersion")
value, type = winreg.QueryValueEx (hKey, "ProductName")
print (' Windows Version      '+Fore.GREEN+Style.BRIGHT+value+Fore.RESET)

hKey = winreg.OpenKey (winreg.HKEY_LOCAL_MACHINE, r"SYSTEM\ControlSet001\Control\Class\{4D36E966-E325-11CE-BFC1-08002BE10318}\0000")
value1, type = winreg.QueryValueEx (hKey, "DriverDesc")
print (' Type                 '+Fore.GREEN+Style.BRIGHT+value1+Fore.RESET)

hKey = winreg.OpenKey (winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows NT\CurrentVersion")
value2, type = winreg.QueryValueEx (hKey, "ProductId")
print (' Serial Number        '+Fore.GREEN+Style.BRIGHT+value2+Fore.RESET)

hKey = winreg.OpenKey (winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows NT\CurrentVersion")
value3, type = winreg.QueryValueEx (hKey, "CSDVersion")
print (' Service Pack         '+Fore.GREEN+Style.BRIGHT+value3+Fore.RESET)

hKey = winreg.OpenKey (winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Internet Explorer")
value4, type = winreg.QueryValueEx (hKey, "svcUpdateVersion")
print (' IE Explorer Version  '+Fore.GREEN+Style.BRIGHT+value4+Fore.RESET)

print(Fore.CYAN+Style.BRIGHT+'--------------------'+Fore.RESET)

# BIOS INFO --------------------------------------------------------------------------------------------
print(Fore.YELLOW+Style.BRIGHT+' BIOS INFO'+Fore.RESET)
print(Fore.CYAN+Style.BRIGHT+'--------------------'+Fore.RESET)

hKey = winreg.OpenKey (winreg.HKEY_LOCAL_MACHINE, r"HARDWARE\DESCRIPTION\System\BIOS")
value5, type = winreg.QueryValueEx (hKey, "BaseBoardManufacturer")
print (' MotherBoard          '+Fore.GREEN+Style.BRIGHT+value5+Fore.RESET)

hKey = winreg.OpenKey (winreg.HKEY_LOCAL_MACHINE, r"HARDWARE\DESCRIPTION\System\BIOS")
value6, type = winreg.QueryValueEx (hKey, "BaseBoardProduct")
print (' Model                '+Fore.GREEN+Style.BRIGHT+value6+Fore.RESET)

hKey = winreg.OpenKey (winreg.HKEY_LOCAL_MACHINE, r"HARDWARE\DESCRIPTION\System\BIOS")
value7, type = winreg.QueryValueEx (hKey, "BIOSVersion")
print (' Bios Version         '+Fore.GREEN+Style.BRIGHT+value7+Fore.RESET)

hKey = winreg.OpenKey (winreg.HKEY_LOCAL_MACHINE, r"HARDWARE\DESCRIPTION\System\BIOS")
value8, type = winreg.QueryValueEx (hKey, "BIOSReleaseDate")
print (' Bios Release Date    '+Fore.GREEN+Style.BRIGHT+value8+Fore.RESET)

hKey = winreg.OpenKey (winreg.HKEY_LOCAL_MACHINE, r"HARDWARE\DESCRIPTION\System\BIOS")
value9, type = winreg.QueryValueEx (hKey, "SystemProductName")
print (' System Product name  '+Fore.GREEN+Style.BRIGHT+value9+Fore.RESET)

hKey = winreg.OpenKey (winreg.HKEY_LOCAL_MACHINE, r"HARDWARE\DESCRIPTION\System\BIOS")
value10, type = winreg.QueryValueEx (hKey, "SystemVersion")
print (' System Version       '+Fore.GREEN+Style.BRIGHT+value10+Fore.RESET)

print(Fore.CYAN+Style.BRIGHT+'--------------------'+Fore.RESET)

# PROCESSOR INFO -----------------------------------------------------------------------------------------
print(Fore.YELLOW+Style.BRIGHT+' PROCESSOR INFO'+Fore.RESET)
print(Fore.CYAN+Style.BRIGHT+'--------------------'+Fore.RESET)

hKey = winreg.OpenKey (winreg.HKEY_LOCAL_MACHINE, r"HARDWARE\DESCRIPTION\System\CentralProcessor\0")
value11, type = winreg.QueryValueEx (hKey, "ProcessorNameString")
print (' Processor Name       '+Fore.GREEN+Style.BRIGHT+value11+Fore.RESET)

print(Fore.CYAN+Style.BRIGHT+'--------------------'+Fore.RESET)

# VIDEO CARD INFO -----------------------------------------------------------------------------------------
print(Fore.YELLOW+Style.BRIGHT+' VIDEO CARD INFO'+Fore.RESET)
print(Fore.CYAN+Style.BRIGHT+'--------------------'+Fore.RESET)

hKey = winreg.OpenKey (winreg.HKEY_LOCAL_MACHINE, r"SYSTEM\ControlSet001\Control\Class\{4D36E968-E325-11CE-BFC1-08002BE10318}\0000")
value12, type = winreg.QueryValueEx (hKey, "Device Description")
print (' Name                 '+Fore.GREEN+Style.BRIGHT+value12+Fore.RESET)

hKey = winreg.OpenKey (winreg.HKEY_LOCAL_MACHINE, r"SYSTEM\ControlSet001\Control\Class\{4D36E968-E325-11CE-BFC1-08002BE10318}\0000")
value13, type = winreg.QueryValueEx (hKey, "DriverVersion")
print (' Driver Version       '+Fore.GREEN+Style.BRIGHT+value13+Fore.RESET)

hKey = winreg.OpenKey (winreg.HKEY_LOCAL_MACHINE, r"SYSTEM\ControlSet001\Control\Class\{4D36E968-E325-11CE-BFC1-08002BE10318}\0000")
value14, type = winreg.QueryValueEx (hKey, "DriverDate")
print (' Driver Date          '+Fore.GREEN+Style.BRIGHT+value14+Fore.RESET)

print(Fore.CYAN+Style.BRIGHT+'--------------------'+Fore.RESET)


# OTHER INFO -----------------------------------------------------------------------------------------
print(Fore.YELLOW+Style.BRIGHT+' OTHER INFO'+Fore.RESET)
print(Fore.CYAN+Style.BRIGHT+'--------------------'+Fore.RESET)

hKey = winreg.OpenKey (winreg.HKEY_LOCAL_MACHINE, r"SYSTEM\ControlSet001\Control\Class\{4D36E96B-E325-11CE-BFC1-08002BE10318}\0000")
value, type = winreg.QueryValueEx (hKey, "DriverDesc")
print (' Keyboard             '+Fore.GREEN+Style.BRIGHT+value+Fore.RESET)

hKey = winreg.OpenKey (winreg.HKEY_LOCAL_MACHINE, r"SYSTEM\ControlSet001\Control\Class\{4D36E96C-E325-11CE-BFC1-08002BE10318}\0000")
value, type = winreg.QueryValueEx (hKey, "DriverDesc")
print (' Audio                '+Fore.GREEN+Style.BRIGHT+value+Fore.RESET)

hKey = winreg.OpenKey (winreg.HKEY_LOCAL_MACHINE, r"SYSTEM\ControlSet001\Control\Class\{4D36E96E-E325-11CE-BFC1-08002BE10318}\0000")
value, type = winreg.QueryValueEx (hKey, "DriverDesc")
print (' Monitor              '+Fore.GREEN+Style.BRIGHT+value+Fore.RESET)

hKey = winreg.OpenKey (winreg.HKEY_LOCAL_MACHINE, r"SYSTEM\ControlSet001\Control\Class\{4D36E96F-E325-11CE-BFC1-08002BE10318}\0000")
value, type = winreg.QueryValueEx (hKey, "DriverDesc")
print (' Mouse                '+Fore.GREEN+Style.BRIGHT+value+Fore.RESET)

# EXIT ---------------------------------------------------------------------------------------------------
print(Fore.CYAN+Style.BRIGHT+'--------------------'+Fore.RESET)

out_file = open("C:/Users/"+getpass.getuser()+"/Desktop/SyNFO OUTPUT/SyNFO OUTPUT.txt","w")
out_file.write('Windows version:      '+value+'\n'+
               'Type:                 '+value1+'\n'+
               'Serial Number:        '+value2+'\n'+
               'SP:                   '+value3+'\n'+
               'MotherBoard:          '+value5+'\n'+
               'Model:                '+value6+'\n'+
               'Bios Version:         '+value7+'\n'+
               'Bios Relase Date:     '+value8+'\n'+
               'System Product Name:  '+value9+'\n'+
               'System Version:       '+value10+'\n'
               'Processor Name:       '+value11+'\n'+
               'Video Card Name:      '+value12+'\n'+
               'Driver Version:       '+value13+'\n'+
               'Driver Date:          '+value14+'\n')
out_file.close()
winsound.PlaySound("C:/Users/"+getpass.getuser()+"/Desktop/SyNFO 0.8/audio/synfo.wav", winsound.SND_ALIAS)

print()
print(' Press '+Fore.RED+Style.BRIGHT+'ENTER'+Fore.RESET+' to exit')
input()
winsound.PlaySound("C:/Users/"+getpass.getuser()+"/Desktop/SyNFO 0.8/audio/synfo-exit.wav", winsound.SND_LOOP+winsound.SND_ASYNC)
time.sleep(1)
exit()
#---------------------------------------------------------------------------------------------------------
